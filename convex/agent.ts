...
// Shared Firecrawl pool: pause paid web fetches when credits run low.
// Email + already-scheduled work keeps flowing; the dashboard shows why.
const CREDIT_FLOOR = 150;
// Per-company cap per cycle so N tenants can't multiply burn linearly.
...
export const runMonitorCycle = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    // global budget guard — one shared credit pool across all tenants
    const minCredits = Math.min(
      ...companies.map((c: any) => c.webResearchCredits ?? Infinity)
    );
    const budgetOk = minCredits > CREDIT_FLOOR;

    for (const company of companies) {
      const fc = (company.webResearchEnabled ?? firecrawl.enabled()) && budgetOk;
      const task = await ctx.runMutation(internal.state.logTask, {
        company: company._id,
        type: "observe",
        status: "running",
        label: "Checking monitored sources for new customer signals",
        detail: budgetOk ? undefined : "Web research paused — credit floor reached",
      });
      try {
        const sources = ((await ctx.runQuery(internal.queries.listSourcesInternal, {
          company: company._id,
        })) as any[]).slice(0, MAX_SOURCES_PER_CYCLE);
        ...
        for (const source of sources) {
          ...
          try {
            items = await fetchSource(source, fc);
          } catch (e: any) {
